...
#
#  Usage:
#    ./install.sh                      # prompts for your key if <BRAND>_API_KEY unset
#    AXE_API_KEY=bnk_... ./install.sh  # (or IMI_API_KEY=bnk_... for the IMI kit)
...
# ── Resolve + validate the key (brand-namespaced env, falling back to AXE_API_KEY) ──
BRAND_KEY_VAR="${BRAND}_API_KEY"
KEY="${!BRAND_KEY_VAR:-${AXE_API_KEY:-}}"
if [ -z "$KEY" ]; then
  printf "Enter your %s memory key (bnk_...): " "$BRAND" >&2; read -r KEY
fi
WHO="$(env "$BRAND_KEY_VAR=$KEY" "$PYBIN" "$KIT_DIR/$CLI_NAME" whoami 2>/dev/null || true)"
